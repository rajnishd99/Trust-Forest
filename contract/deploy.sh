...
: "${DEPLOYER_IDENTITY:?Set DEPLOYER_IDENTITY to funded testnet identity name or secret}"
: "${ADMIN_ADDRESS:?Set ADMIN_ADDRESS to the fixed admin address}"
...
DEPLOYER_ADDRESS="$(resolve_address "${DEPLOYER_IDENTITY}")"
if [[ "${ADMIN_ADDRESS}" != "GCGEXUG76FMVLCQHMVEUIQ2GPDEZSSNXQZQITISFUR433LZCD4UPGMYT" ]]; then
  echo "ADMIN_ADDRESS must equal fixed admin GCGEXUG76FMVLCQHMVEUIQ2GPDEZSSNXQZQITISFUR433LZCD4UPGMYT." >&2
  exit 1
fi
...
update_env_value "DEPLOYER_IDENTITY" "${DEPLOYER_IDENTITY}"
